...
B <- 999
strip.count <- max(tee.data$Sample.Label)   # number of transects 
N.hats <- NULL                                                    
...
for (i in 1:B) {
  ...
  tee.model <- ddf(method = 'trial.fi', mrmodel=~glm(link='logit', formula=~distance+size+exposure), 
                   data = boot.sample, meta.data=list(width=4))
  N.hats[i] <- summary(tee.model)[9]$Nhat   # store abundance estimate
}

## abundance estimate from original sample
tee.model.orig <- ddf(method = 'trial.fi', mrmodel=~glm(link='logit', formula=~distance+size+exposure), 
                 data = tee.data, meta.data=list(width=4))
N.hats[1000] <- summary(tee.model.orig)[9]$Nhat
